...
pub mod pallet {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		///
		/// By implementing `fn offchain_worker` you declare a new offchain worker. This function
		/// will be called when the node is fully synced and a new best block is successfully
		/// imported.
		///
		/// Note that it's not guaranteed for offchain workers to run on EVERY block, there might be
		/// cases where some blocks are skipped, or for some the worker runs twice (re-orgs), so the
		...
		fn offchain_worker(block_number: BlockNumberFor<T>) {
			...
			// in WASM. The `sp-api` crate also provides a feature `disable-logging` to disable all
			// logging and thus, remove any logging from the WASM.
			log::info!("Hello World from offchain workers!");

			// Since off-chain workers are just part of the runtime code, they have direct access to
			// the storage and other included pallets.
			//
			...
			// of the code to separate `impl` block. Here we call a helper function to calculate
			// current average price. This function reads storage entries of the current state.
			...
			// The result of `mutate` call will give us a nested `Result` type. The first one
			// matches the return of the closure passed to `mutate`, i.e. if we return `Err` from
			// the closure, we get an `Err` here. In case we return `Ok`, here we will have another
			// (inner) `Result` that indicates if the value has been set to the storage correctly -
			// i.e. if it wasn't written to in the meantime.
			match res {
				...
				// most likely executed the same logic and succeeded at writing to storage. Thus we
				// don't really want to send the transaction, knowing that the other run already
				// did.
	...
	impl<T: Config> Pallet<T> {
		...
		///
		/// This method is a public function of the module and can be called from within a
		/// transaction. It appends given `price` to current list of prices. In our example the
		/// `offchain worker` will create, sign & submit a transaction that calls this function
		/// passing the price.
		///
...
impl<T: Config> Pallet<T> {
	...
	fn fetch_price_and_send_signed() -> Result<(), &'static str> {
		...
		// Make an external HTTP request to fetch the current price. Note this call will block until
		// response is received.
		let price = Self::fetch_price().map_err(|_| "Failed to fetch price")?;

		// Using `send_signed_transaction` associated type we create and submit a transaction
		// representing the call, we've just created. Submit signed will return a vector of results
		// for all accounts that were found in the local keystore with expected `KEY_TYPE`.
